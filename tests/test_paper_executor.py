...
def test_buy_leg_deducts_cash_and_fees(executor):
    ...
    assert "KXTEST-1:YES" in executor.portfolio.positions
...
def test_buy_then_sell_realizes_pnl(executor):
    ...
    assert "K1:YES" not in executor.portfolio.positions
...
def test_buy_more_blends_avg_price(executor):
    ...
    pos = executor.portfolio.positions["K1:YES"]
...
def test_execute_arb_buy_both(executor):
    ...
    assert len(fills) == 2
    assert "K1:YES" in executor.portfolio.positions
    assert "K1:NO" in executor.portfolio.positions
    # We hold both sides — guaranteed $1/contract at settlement
    yes_pos = executor.portfolio.positions["K1:YES"]
    no_pos = executor.portfolio.positions["K1:NO"]
